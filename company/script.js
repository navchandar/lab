...
async function loadData() {
  ...
  try {
    ...
    data.forEach((item) => {
      ...
      if (!item.employee_count && !item.website && !item.ln_employee_count) {
        return;
      }
      ...
      if (item.ln_employee_count && item.ln_employee_count !== "0") {
        const normalized = normalizeEmployeeSize(item.ln_employee_count);
        displayCount = normalized.display;
        sortRank = normalized.rank;
      } else if (item.employee_count && item.employee_count !== "-") {
        displayCount = item.employee_count;
